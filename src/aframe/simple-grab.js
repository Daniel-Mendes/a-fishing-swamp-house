...
AFRAME.registerComponent("simple-grab-drop-zone", {
  schema: {
    ...
    dropMultiple: { type: "boolean", default: false },
    event: { type: "string", default: "click" },
    dynamic: { type: "boolean", default: false },
    allowed: { type: "selectorAll", default: "*" },
  },
  ...
  onEvent: function (evt) {
    ...
    // disallow dropping if the grabbed object is not allowed
    if (
      this.data.allowed !== "*" &&
      currentGrab !== null &&
      !this.data.allowed.includes(currentGrab)
    )
      return;

    // disallow dropping if the drop zone is already occupied
    if (
      !this.data.dropMultiple &&
      this.data.dropOnly &&
      this.droppedEl !== null
    )
      return;
    ...
    // if the event is from a hand, emit a custom event
